...
const productSchema = z.object({
  ...
  minStock: z.string().regex(/^\d+(\.\d{1,6})?$/, "Минимум на складе ГП"),
  maxStock: z.string().regex(/^\d+(\.\d{1,6})?$/, "Лимит на складе ГП"),
  laborRate: z.string().regex(/^\d+(\.\d{1,4})?$/, "Оплата рабочему за м²"),
...
export async function createProduct(formData: FormData) {
  ...
  const parsed = productSchema.safeParse({
    ...
    minPrice: formData.get("minPrice") || formData.get("price") || "0",
