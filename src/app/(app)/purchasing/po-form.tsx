...
export function PurchaseOrderForm({
  ...
  return (
    <form action={submit} className="space-y-3 rounded-2xl border border-[var(--line)] bg-white p-5">
      ...
      <button type="button" className="text-sm text-teal-800" onClick={() => setRows([...rows, { materialId: materials[0]?.id ?? "", quantity: "", unitPrice: "" }])}>
        + позиция
      </button>
      ...
      <button className="rounded-lg bg-teal-800 px-4 py-2 text-sm text-white">Создать заявку</button>
    </form>
  );
}
