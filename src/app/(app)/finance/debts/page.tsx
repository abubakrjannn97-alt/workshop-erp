...
import { formatPurchaseOrderNo, orderNo, shortProductLabel } from "@core/shared/format";
  ...
  const customerDebts = orders
    .map((o) => {
      ...
      return {
        ...
        orderLabel: orderNo(o.number),
        debt: money(debt),
        lines: o.items.map((item) => ({
          label: shortProductLabel(item.product.name),
          detail: `${qtyDisplay(item.quantity)} ${item.product.saleUnit?.symbol ?? "м²"}`.trim(),
        })),
      };
    })
