...
export default async function SettingsPage() {
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.section} data-tour="set-form">
        ...
        <div className={styles.sectionBody}>
          <form action={updateBusinessSettings} className="grid max-w-xl gap-3">
            <FormField label={t("set.companyName")}><input name="companyName" defaultValue={values.companyName} disabled={!canEdit} className="ui-input" /></FormField>
            <FormField label={t("set.logoUrl")}><input name="logoUrl" defaultValue={values.logoUrl} disabled={!canEdit} className="ui-input" /></FormField>
            <FormField label={t("set.currencyCode")}><input name="currencyCode" defaultValue={values.currencyCode} disabled={!canEdit} className="ui-input" /></FormField>
            <FormField label={t("set.currencyName")}><input name="currencyName" defaultValue={values.currencyName} disabled={!canEdit} className="ui-input" /></FormField>
            <FormField label={t("set.timezone")}><input name="timezone" defaultValue={values.timezone} disabled={!canEdit} className="ui-input" /></FormField>
            <FormField label={t("set.discountLimit")}><input name="discountLimitPercent" defaultValue={values.discountLimitPercent} disabled={!canEdit} className="ui-input" inputMode="decimal" /></FormField>
            <FormField label={t("set.expenseReserve")}><input name="opexReservePercent" defaultValue={values.opexReservePercent} disabled={!canEdit} className="ui-input" inputMode="decimal" /></FormField>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHead}><h2 className={styles.sectionTitle}>{t("set.orderStatuses")}</h2></div>
        <div className={styles.sectionBody}>
            ...
            {orderStatuses.map((s) => (
              <li key={s.id} style={{ padding: "6px 0", borderBottom: "1px solid var(--line)" }}>
                <form action={renameOrderStatus} className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  <span style={{ width: 140, flexShrink: 0, fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--ink-3)" }}>{s.code}</span>
                  <input name="name" defaultValue={s.name} disabled={!canEdit} className="ui-input min-w-[10rem] flex-1" />
                  {canEdit ? <button type="submit" className="min-h-[44px] text-sm font-medium" style={{ color: "var(--accent)" }}>{t("common.save")}</button> : null}
                </form>
              </li>
            ))}
          </ul>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHead}><h2 className={styles.sectionTitle}>{t("set.pipelineStages")}</h2></div>
        <div className={styles.sectionBody}>
          <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
            {leadStages.map((s) => (
              <li key={s.id} style={{ padding: "6px 0", borderBottom: "1px solid var(--line)" }}>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHead}><h2 className={styles.sectionTitle}>{session.user.name}</h2></div>
        <div className={styles.sectionBody}>
